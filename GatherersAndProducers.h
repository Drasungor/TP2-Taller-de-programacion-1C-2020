...
private:
  ...
  int _load_resources(std::ifstream& resources,
  ...
  int _add_worker_ammount(std::map<Gatherer, int>& gatherers_ammounts,
                           std::map<Producer, int>& producers_ammounts,
                           std::string& worker, std::string& ammount) const;
  int _load_workers_ammounts(std::ifstream& workers,
